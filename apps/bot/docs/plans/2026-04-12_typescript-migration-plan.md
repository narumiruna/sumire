...
Reimplement `telegramagent` under `./apps/bot` with behavior-compatible Telegram handling while keeping the Python implementation available until the TypeScript service passes its parity checks.
...
The TypeScript implementation is isolated in `./apps/bot`. Python source, deployment files, and runtime data remain unchanged until cutover.
...
- [x] Create the Node/TypeScript/Biome/Vitest project in `./apps/bot`; evidence: dependency install and all local quality scripts pass.
