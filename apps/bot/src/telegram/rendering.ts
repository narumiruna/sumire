  ...
  return Array.from(text.replace(/\r\n?/g, "\n"))
    .filter((character) => {
      ...
      return (
        character === "\n" ||
        character === "\t" ||
        (codePoint >= 0x20 && !(codePoint >= 0xd800 && codePoint <= 0xdfff))
      )
    })
    .join("")
}
...
// Derive the text Telegram shows from the HTML generated by this module.
export function telegramVisibleText(text: string): string {
  return (telegramHtmlChunks(text)[0] ?? " ")
    .replace(/<a href="[^"]*">|<\/?(?:a|b|code|pre)>/gu, "")
    .replaceAll("&lt;", "<")
    .replaceAll("&gt;", ">")
    .replaceAll("&amp;", "&")
}
