...
The `load_public_url` tool remains available by default; Pi advertises its bundled `load-public-url` skill when coding tools are enabled. The separate `load-url-content` CLI skill from the URL-content package is also available only when `BOT_CODING_TOOLS_ENABLED=true` and the chat is allowlisted; it needs the `bash` tool and does not change the bot's default permissions.
