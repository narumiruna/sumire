...
function requireTextContentType(contentType: string, loaderName: string, url: string): void {
  ...
  throw new LoaderContentError(
    loaderName,
    url,
    `Expected textual content, got: ${JSON.stringify(contentType)}`,
  )
}
  ...
  try {
    ...
    for (let redirects = 0; redirects <= 5; redirects += 1) {
      ...
      if ([301, 302, 303, 307, 308].includes(response.status)) {
        ...
        if (redirects === 5)
          throw new LoaderContentError(loaderName, url, "URL exceeded the redirect limit")
        if (!location)
          throw new LoaderContentError(
            loaderName,
            url,
            "URL redirect did not include a Location header",
          )
...
export async function fetchImpersHtml(
  url: string,
  options: ImpersFetchOptions = {},
): Promise<RetrievedHtml> {
  const response = await fetchImpersResponse(url, {
    ...options,
