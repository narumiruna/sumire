...
export function decodeSessionExport(html: string, sourceUrl: string): JsonRecord {
  ...
  if (!encoded)
    throw contentError(sourceUrl, 'Missing embedded <script id="session-data"> payload.')
  try {
...
function selectedEntries(
  session: JsonRecord,
  requestedLeafId: string | undefined,
  sourceUrl: string,
): JsonRecord[] {
  if (!Array.isArray(session.entries))
    throw contentError(sourceUrl, "Embedded session data has no entries list.")
  ...
  for (const raw of session.entries) {
    ...
    if (!entry || !id)
      throw contentError(sourceUrl, "Embedded session data contains an invalid entry.")
    if (byId.has(id))
      throw contentError(
        sourceUrl,
        `Embedded session data contains duplicate entry ID ${JSON.stringify(id)}.`,
      )
    entries.push(entry)
    byId.set(id, entry)
  }
  ...
  if (!byId.has(leafId))
    throw contentError(sourceUrl, `Session leaf ${JSON.stringify(leafId)} was not found.`)
  ...
  while (currentId) {
    if (visited.has(currentId))
      throw contentError(sourceUrl, "Embedded session entry ancestry contains a cycle.")
    visited.add(currentId)
    const entry = byId.get(currentId)
    if (!entry)
      throw contentError(
        sourceUrl,
        `Session parent entry ${JSON.stringify(currentId)} was not found.`,
      )
    selected.push(entry)
    if (entry.parentId === null || entry.parentId === undefined) currentId = undefined
      ...
      throw contentError(sourceUrl, `Entry ${JSON.stringify(currentId)} has an invalid parent ID.`)
  }
...
function renderNonMessage(entry: JsonRecord): string | undefined {
  ...
  if (entryType === "thinking_level_change") {
    return [
      ...
      ...metadata(entry),
      label(entry.thinkingLevel, "unknown"),
    ].join("\n\n")
  }
  if (entryType === "compaction") {
    const tokens = Number.isInteger(entry.tokensBefore)
      ? `- Tokens before: ${String(entry.tokensBefore)}`
      : ""
    return ["### Compaction", ...metadata(entry, tokens), text(entry.summary).trim()]
      .filter(Boolean)
      .join("\n\n")
  }
  if (entryType === "branch_summary") {
    return ["### Branch Summary", ...metadata(entry), text(entry.summary).trim()]
      .filter(Boolean)
      .join("\n\n")
  }
  if (entryType === "custom_message" && entry.display === true) {
    const body =
      typeof entry.content === "string"
        ? entry.content
        : JSON.stringify(entry.content, undefined, 2)
    return [`### ${label(entry.customType, "Custom Message")}`, ...metadata(entry), body]
      .filter(Boolean)
      .join("\n\n")
  }
  return undefined
...
export function renderPiSessionMarkdown(
  session: JsonRecord,
  options: { sourceUrl: string; leafId?: string },
): string {
  ...
  if (text(session.systemPrompt).trim())
    sections.push("## System Prompt", text(session.systemPrompt).trim())
  ...
  const rendered = selectedEntries(session, options.leafId, options.sourceUrl).flatMap((entry) => {
    const value =
      text(entry.type) === "message"
        ? renderMessage(entry, record(entry.message) ?? {})
        : renderNonMessage(entry)
    return value ? [value] : []
  })
...
function gistFile(gist: unknown, target: PiSessionTarget, sourceUrl: string): JsonRecord {
  ...
  if (!file)
    throw contentError(sourceUrl, `Gist does not contain ${JSON.stringify(target.fileName)}.`)
  return file
}
    ...
    try {
      ...
      if (file.truncated === true || !content) {
        content = await readResponseText(
          await this.get(validatedRawUrl(file.raw_url, url), {
            redirect: "follow",
            signal: activeSignal,
          }),
      ...
      return renderPiSessionMarkdown(decodeSessionExport(content, url), {
        sourceUrl: url,
