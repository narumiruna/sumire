...
describe("source loaders", () => {
  it("normalizes GitHub blob URLs and preserves raw content", async () => {
    ...
    await expect(
      new GitHubLoader({ resources }).load("https://github.com/a/b/blob/main/demo.ts"),
  ...
  it("times out stalled GitHub requests", async () => {
    ...
    await expect(
      new GitHubLoader({ resources, timeoutMs: 5 }).load(
  ...
  it("rejects non-text responses before generic Markdown conversion", async () => {
    ...
    await expect(
      new CurlCffiLoader({ resources }).load("https://example.com/image"),
    ).rejects.toThrow("Expected textual content")
  })
    ...
    const session = {
      get: async (_url: string, options: Record<string, unknown>) => {
        ...
        return {
          ...
          headers: {
            get: (name: string) =>
              request === 1 && name === "location" ? "https://other.example/final" : null,
          },
          setContent: () => undefined,
          close: async () => undefined,
        }
  ...
  it("aborts an impers response while it exceeds the byte limit", async () => {
    ...
    await expect(
      fetchImpersResponse("https://example.com/file.pdf", { session, maxBytes: 3 }),
  ...
  it("times out stalled HTTP requests by default", async () => {
    ...
    await expect(
      new HttpLoader({ resources, timeoutMs: 5 }).load("https://example.com"),
    ).rejects.toBeInstanceOf(LoaderTimeoutError)
  })
  ...
  it("treats mixed-case HTTP schemes as remote PDF URLs", async () => {
    ...
    await expect(new PdfLoader({ resources }).load("HTTPS://example.com/file.pdf")).rejects.toThrow(
      "Not a PDF file",
    )
  ...
  it("rejects remote PDFs that declare an oversized response", async () => {
    ...
    await expect(
      new PdfLoader({ resources }).load("https://example.com/oversized.pdf"),
    ).rejects.toThrow(`Response exceeds the ${MAX_PDF_BYTES} byte limit`)
  })
  ...
  it("bounds Reddit RSS and JSON responses before parsing", async () => {
    ...
    await expect(
      new RedditLoader({ resources }).load("https://reddit.com/comments/abc/post"),
  ...
  it("rejects blocker pages and empty Reddit feeds", () => {
    expect(() =>
      rssToMarkdown("<html><body>verify</body></html>", "https://www.reddit.com/post/.rss"),
    ).toThrow("not an Atom feed")
    expect(() =>
      rssToMarkdown("<feed><title>Empty</title></feed>", "https://www.reddit.com/post/.rss"),
    ).toThrow("no entries")
  })
  ...
  it("loads and verifies Twitter status content through FxTwitter", async () => {
    ...
    const result = await new TwitterLoader({ resources }).load(
      `https://x.com/example/status/${statusId}`,
    )
    ...
    expect(
      renderFxTwitterPayload(payload, statusId, "https://x.com/example/status/123456789"),
    ).toBe(result)
  })
  ...
  it("rejects an FxTwitter response for a different status", () => {
    expect(() =>
      renderFxTwitterPayload(
        { tweet: { id: "other" } },
        "requested",
        "https://x.com/example/status/requested",
      ),
